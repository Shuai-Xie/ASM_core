...
from datasets.voc.VOC import VOC_Dataset
from datasets.voc.voc_parser import load_data
from datasets.transforms import get_transform

from net.detector.faster_rcnn import get_model
from tools.utils import collate_fn
from tools.engine import train_one_epoch, evaluate

from main.det.select_criteria import detect_unlabel_imgs
...
def train_model(dataloader, start_epoch, ap_range, ap_shift_thre, asm=True):
    ...
    for epoch in range(start_epoch, args.num_epoches):  # epoch 要从0开始，内部有 warm_up
        ...
        if evals[args.ap] > args.ap_thre:
            ...
            save_model(ckpt_path, model, epoch, evals[args.ap], optimizer)
            ...
            if asm:
                dataloader = update_asm_dataloader(writer, epoch + 1)
...
if __name__ == '__main__':
    ...
    # tensorbaord 检测参数
    writer = SummaryWriter(log_dir='runs/det_{}_{}'.format(args.backbone, get_curtime()))
    ap_records = {'ap_50': [], 'ap_75': [], 'ap_shift': []}

    # model save
    model_save_dir = os.path.join('output', 'det_{}_{}'.format(args.backbone, get_curtime()))
    os.makedirs(model_save_dir, exist_ok=True)

    if self_pretrained:
        print('load pretrain model')
        model, optimizer, best_epoch, best_acc = load_model(model, args.ckpt, optimizer)
        # lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.5, last_epoch=best_epoch)
