...
def update_asm_dataloader(writer, epoch):
    ...
    """
    global DU, DL, DH  # pool, init_label, high_conf
    ...
    # label + uncertain, 'tuple' object does not support item assignment
    ...
    if args.cost_effective:  # 使用 high confidence
        hc_idxs, hc_labels = get_high_confidence_samples(y_pred_porb, delta_scheduler.delta)  # entropy thre
        ...
        if len(hc) > 0:
            DH = np.take(DU[0], hc[:, 0], axis=0), hc[:, 1]  # hc_imgs, hc_labels
            hc_gt_labels = np.take(DU[1], hc[:, 0], axis=0)
            hc_accuracy = len(np.where(DH[1] == hc_gt_labels)[0]) / len(hc_gt_labels)
    ...
    x_train, y_train = np.concatenate((DL[0], DH[0])), np.concatenate((DL[1], DH[1]))
    ...
    # 更新完 dataloader
    # 更新 DU: uncertain samples 标注后，加入 DL，并从 DU 删除
    # 恢复 DH: erase SL dataset
    DH = empty_x(), empty_y()
